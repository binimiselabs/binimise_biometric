...
A fully **offline, on-device** Flutter SDK for face liveness detection and identity verification.  
No server. No internet. All inference runs locally on the device.
...
dependencies:
  binimise_biometric:
    git:
      url: https://github.com/binimiselabs/binimise_biometric.git
...
void main() async {
  ...
  await LivenessFlutter.initialize(licenseKey: 'YOUR_LICENSE_KEY');
...
```dart
final LivenessResult? result = await LivenessFlutter.startLiveness(context);
...
final liveness = await LivenessFlutter.startLiveness(context);
if (liveness == null || !liveness.isReal) return;

final FaceMatchResult? match = await LivenessFlutter.matchFaceFromImage(
  liveness.imageBytes!,
...
final FaceMetadataResult? meta = await LivenessFlutter.generateFaceMetadata(imageBytes);
...
```dart
final double score = LivenessFlutter.compareFaceMetadata(meta1.metadata, meta2.metadata);
...
await LivenessFlutter.startLiveness(context);

// Override
await LivenessFlutter.startLiveness(
  context,
...
- Email: [support@binimise.com](mailto:support@binimise.com)
- Phone: 080-50977232
